...
export function New() {
  ...
  function onSubmit(data: FormData) {
    if(carImages.length === 0) {
      alert("Envie pelo menos uma imagem do carro");
      return;
    }
    ...
    addDoc(collection(db, "cars"), {
      ...
      console.log(error)
      console.log("Erro ao enviar os dados do carro")
    });
  }
  ...
  async function handleUpload(image: File) {
    ...
    const storageRef = ref(storage, `images/${currentUid}/${uidImage}`);
  ...
  return (
    <Container>
      ...
      <div className="w-full bg-white p-3 rounded-lg flex flex-col sm:flex-row items-center gap-2">
        ...
        {carImages.map((item) => (
          <div key={item.name} className="w-full h-32 flex items-center">
            <img
              src={item.previewUrl}
              alt="Foto do carro"
